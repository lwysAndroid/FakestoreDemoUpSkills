...
import com.chemasmas.fakestoreapi.presentation.features.login.mocks.PerformLoginRepositoryMock
...
class LoginViewModelTest {
    ...
    private val performLoginRepositoryMock = PerformLoginRepositoryMock()

    private val viewModel = LoginViewModel(
        dispatchersSource = dispatchersSource,
        validateEmailUseCase = ValidateEmailUseCase(),
        validatePasswordUseCase = ValidatePasswordUseCase(),
        performLoginUseCase = PerformLoginUseCase(performLoginRepositoryMock),
    )
    ...
    fun `Login with invalid email`() {
        viewModel.performLogin(email = "someEmail@gmail.", password = "")
        dispatcher.scheduler.advanceUntilIdle()
        val invalidEmail = viewModel.state.value.errorUser
        assertEquals(R.string.invalid_email, invalidEmail)
    }
    ...
    fun `Login with empty password`() {
        viewModel.performLogin(email = "someEmail@gmail.com", password = "")
        dispatcher.scheduler.advanceUntilIdle()
    ...
    @Test
    fun `Correct login`() {
        viewModel.performLogin(email = "someEmail@gmail.com", password = "1234password")
        dispatcher.scheduler.advanceUntilIdle()
